 ...
 *
 * <p>A {@code Solver} bundles all functionality on how to solve a problem, but no specifics of the problem being solved.
 * The advantages and disadvantages of a specific {@code Solver} depend on its implementation.</p>
 *
...
public interface Solver<U extends SearchNode, V> {
     ...
     *
     * <p>The search will explore the search graph defined by the starting state, environment and state generator. Any
     * found goal nodes will be reported to the {@code Manager}. The provided heuristic may be used to guide or limit
     * the areas of the search graph being visited. It is assumed the provided heuristic is admissible unless otherwise
     * mentioned.</p>
     *
     * @param startNodes the starting nodes from which the search state space will be constructed
